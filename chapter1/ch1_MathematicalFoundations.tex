...
For example, let $s=\{1,2,3,4,5\}$ and the predicate $P\left(x\right)$ be true whenever $x$ is even.  Then, we have the set $y = \{x \in s | P (x) \textit{true for } $x$ \textit{ even} \} = \{2,4\}$.
