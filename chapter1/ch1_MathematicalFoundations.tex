...
If we compare this to the truth table for implication, we see that implication is always true.  Note that with this particular definition of the statements $A$ and $B$ we cannot have a case where $A$ is false and $B$ is true. \\ 
...
\item \textit{Contradiction}:  Any state that is false for all values of its components is a contradiction.  The negation of a contradiction is a tautology and the negation of a tautology is a contradiction.
