...
\textit{Composition} \index{composition} of functions is an important concept that appears quite frequently in some of the later topics.  The notation for this is $(f \circ g)(x)=f(g(x))$, where $f$ and $g$ are functions, $x$ is an element of a set, and the circle indicates composition.  In words, we can think of this as a chain, whereby, firstly the mapping is applied using the the $g$ function, then the result of this is mapped via the $f$ function.  Suppes provides a formal definition of composition using something called the \textit{relative product}\index{relative product}, so for completeness we provide the definitions:\\
